...
def test_multiple_permissions_any_match(client, db, regular_user):
...
def test_multiple_roles_with_match(admin_client):
...
def test_all_permissions_unauthenticated(client):
    """Test that decorator returns 401 for unauthenticated users."""
    response = client.get('/test_all_permissions')
    assert response.status_code == 401

def test_all_permissions_without_all(client, db, regular_user):
...
def test_all_permissions_with_all(client, db, user, role_editor):
...
def test_all_permissions_with_admin(admin_client):
...
def test_all_roles_unauthenticated(client):
    """Test that decorator returns 401 for unauthenticated users."""
    response = client.get('/test_all_roles')
    assert response.status_code == 401

def test_all_roles_without_all(client, db, regular_user):
...
def test_all_roles_with_all(client, db, user, role_user):
...
def test_all_roles_with_only_one(admin_client):
...
def test_stacked_decorators(app, admin_client):
...
def test_permission_check_case_sensitivity(client, db, user, role_user, permission_view_users):
...
def test_role_check_order_independence(client, db):
...
def test_empty_permission_name(client, admin_user):
...
def test_none_permission_name(admin_user):
