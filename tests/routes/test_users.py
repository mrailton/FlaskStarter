...
class TestUsersIndex:
    ...
    def test_users_requires_permission(self, authenticated_client):
        """Test that users list requires view users permission."""
        response = authenticated_client.get('/users/')
        assert response.status_code == 403  # Forbidden

    def test_users_list_with_permission(self, admin_client, db):
        """Test that users with permission can view list."""
        response = admin_client.get('/users/')
        assert response.status_code == 200

    def test_users_list_shows_users(self, admin_client, db, regular_user):
        """Test that users list shows all users."""
        response = admin_client.get('/users/')
        assert response.status_code == 200
        assert b'Users' in response.data or b'users' in response.data

    def test_users_search(self, admin_client, db):
    ...
    def test_users_pagination(self, admin_client, db):
...
class TestUsersCreate:
    ...
    def test_create_user_success(self, admin_client, db):
    ...
    def test_create_user_with_roles(self, admin_client, db, role_user):
    ...
    def test_create_user_without_password(self, admin_client, db):
    ...
    def test_create_duplicate_email(self, admin_client, user):
...
class TestUsersEdit:
    ...
    def test_edit_user_success(self, admin_client, db, regular_user):
    ...
    def test_edit_user_email(self, admin_client, db, regular_user):
    ...
    def test_edit_user_password(self, admin_client, db, regular_user):
    ...
    def test_edit_user_roles(self, admin_client, db, regular_user, role_editor):
    ...
    def test_edit_duplicate_email(self, admin_client, user, regular_user):
...
class TestUsersDelete:
    ...
    def test_delete_requires_authentication(self, client, user):
        """Test that deleting user requires login."""
        response = client.post(f'/users/{user.id}/delete')
        assert response.status_code in [302, 401]

    def test_delete_requires_permission(self, authenticated_client, regular_user):
...
class TestUsersAPI:
    ...
    def test_api_returns_users_list(self, admin_client, db, regular_user):
    ...
    def test_api_search(self, admin_client, db, regular_user):
