...
def test_create_permission_success(admin_client, db):
...
def test_create_duplicate_permission_name(admin_client, permission_view_users):
...
def test_create_permission_short_name(admin_client):
...
def test_create_permission_empty_name(admin_client):
...
def test_edit_requires_authentication(client, permission_view_users):
    """Test that editing permission requires login."""
    response = client.get(f'/permissions/{permission_view_users.id}/edit')
    assert response.status_code in [302, 401]

def test_edit_requires_permission(authenticated_client, permission_view_users):
...
def test_edit_form_loads(admin_client, permission_view_users):
...
def test_edit_permission_name(admin_client, db):
...
def test_edit_nonexistent_permission(admin_client):
    """Test editing non-existent permission."""
    response = admin_client.get('/permissions/99999/edit', follow_redirects=True)
    assert b'not found' in response.data

def test_edit_duplicate_name(admin_client, permission_view_users, permission_edit_users):
...
def test_edit_permission_affects_roles(admin_client, db, role_user, permission_view_users):
...
def test_delete_requires_permission(authenticated_client, permission_edit_users):
...
def test_delete_permission_success(admin_client, db):
...
def test_delete_permission_removes_role_associations(admin_client, db, role_user, permission_view_users):
...
def test_delete_nonexistent_permission(admin_client):
    """Test deleting non-existent permission."""
    response = admin_client.post('/permissions/99999/delete', follow_redirects=True)
    assert b'not found' in response.data

def test_delete_permission_affects_users(admin_client, db, regular_user, role_user, permission_view_users):
...
def test_api_returns_permissions_list(admin_client, db, all_permissions):
...
def test_api_includes_roles_count(admin_client, db, permission_view_users, role_user):
...
def test_api_search(admin_client, db, permission_view_users):
