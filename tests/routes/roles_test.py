...
def test_create_role_success(admin_client, db):
...
def test_create_role_with_permissions(admin_client, db, permission_view_users, permission_edit_users):
...
def test_create_duplicate_role_name(admin_client, role_user):
...
def test_create_role_short_name(admin_client):
...
def test_edit_role_name(admin_client, db, role_editor):
...
def test_edit_role_permissions(admin_client, db, role_editor, permission_delete_users):
...
def test_edit_role_add_permissions(admin_client, db):
...
def test_edit_nonexistent_role(admin_client):
    """Test editing non-existent role."""
    response = admin_client.get('/roles/99999/edit', follow_redirects=True)
    assert b'not found' in response.data

def test_edit_duplicate_name(admin_client, role_user, role_editor):
...
def test_delete_role_removes_user_associations(admin_client, db, regular_user, role_user):
...
def test_delete_nonexistent_role(admin_client):
...
def test_api_returns_roles_list(admin_client, db, role_user, role_editor):
...
def test_api_includes_counts(admin_client, db, role_user, regular_user):
...
def test_api_search(admin_client, db, role_user):
